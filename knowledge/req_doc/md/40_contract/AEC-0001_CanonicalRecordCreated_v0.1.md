# Async Event Contract — Canonical Record Created

| Field | Value |
|-------|-------|
| Document ID | AEC-0001 |
| Title | Canonical Record Created |
| Version | v0.1 |
| Status | Draft |
| Created | 2026-03-01 |
...
| Field | Value |
|-------|-------|
| Event Name | `canonical.record.created` |
| Domain | canonical |
| Event Type | notification |
| Trigger | A new canonical instrument record is persisted — either manually by a Data Owner via the Operator UI, or automatically by Source Processing from a golden source |

**Business Description**

Signals to downstream systems that a new instrument identity is available in the canonical entity table and can be referenced for ingestion, enrichment, and export. Consumers should call the Canonical Query API (PSD-0002 FLOW-A) using the canonical_key to retrieve the full record.
...
|------|---------|---------|
| NMD Team | Data Ingestion Module (FRD-NMD-0002) | Detect new canonical records to initialise source-to-canonical mappings and ingestion pipelines |
...
| Field | Value |
|-------|-------|
| Topic | `nmd.canonical.record.created` |
...
|-------|------|----------|-------------|
| `canonical_key` | string | Yes | The canonical key hash of the newly created record. Consumers use this to call the Canonical Query API for the full record. |
...
|-------------|----------------|
| `payload.canonical_key` | Use as lookup key to call Canonical Query API (PSD-0002 FLOW-A) and fetch full record before initialising source mappings |
...
| Versioning | Breaking changes result in a new event type (e.g., canonical.record.created.v2) published in parallel until all consumers have migrated |
...
| Dead Letter Topic | `nmd.canonical.record.created.dlt` — failed consumer messages routed here after max retries |
...
| Consume Permission | Data Ingestion Module (FRD-NMD-0002) only |
...
|---------|------|--------|---------| 
| v0.1 | 2026-03-01 | Leo | Initial draft — lightweight notification event carrying canonical_key only; consumers call Canonical Query API for full record |
