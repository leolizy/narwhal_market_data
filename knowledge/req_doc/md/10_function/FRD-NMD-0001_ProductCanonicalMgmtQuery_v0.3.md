...
9. Predefined entity type definitions per asset class
...
| Entity Type | A predefined canonical record shape for a given asset class, specifying which hierarchy columns are populated and the permitted contract column format (e.g., Future Contract, Option Contract) |
...
### FA-03: Asset Class Canonical Definitions

| ID | Requirement | Priority | Traces To | Acceptance Criteria |
|----|-------------|----------|-----------|---------------------|
| FR-NMD-001-015 | System shall predefine the following entity types for the Futures & Options asset class, specifying which hierarchy columns are populated and the permitted contract column formats. Columns 3-4 (Product Type, Product Code) are null-placeholder for entity types that do not use them. Product Type must be one of: PHY (Physical), FUT (Futures), CMB (Combination), OOP (Option on Physical), OOF (Option on Future), OOC (Option on Combination).

Entity types:
- Exchange: Exchange only (L1); columns 2-5 null
- Commodity: Exchange + Commodity (L1-L2); columns 3-5 null
- Product: Exchange + Commodity + Product Type + Product Code (L1-L3); column 5 null
- Future Contract: all 5 columns; contract format: YYYYMM | YYYYMMDD | YYYYMMWN
- Option Reference Contract: all 5 columns; contract format: YYYYMM | YYYYMMDD | YYYYMMWN
- Option Contract: all 5 columns; contract format: YYYYMM_CP_STRIKE | YYYYMMDD_CP_STRIKE | YYYYMMWN_CP_STRIKE where CP = C or P (call/put) and STRIKE = strike price | Must | FR-001 | All 6 Futures & Options entity types are representable in the canonical table; contract format validated per entity type on create/update; Option Contract format follows YYYYMM_CP_STRIKE pattern where CP is C or P and STRIKE is the strike price; Product Type values restricted to the defined set; canonical key hash computed with null placeholders for unpopulated columns |
| FR-NMD-001-016 | System shall predefine the following entity type for the FX asset class. Exchange column is fixed as 'FX' (placeholder). Columns 3-5 (Product Type, Product Code, Contract) are null for all FX records. No Product Type enumeration is defined for FX in Phase 1.

Entity types:
- FX Currency Pair: Exchange = 'FX' (fixed placeholder), Commodity = XXXYYY (standardised 6-character currency pair, no separator); columns 3-5 null | Must | FR-001 | FX Currency Pair records are representable in the canonical table with Exchange fixed as 'FX' and Commodity in XXXYYY format; columns 3-5 are null; currency pair format validated on create/update |
...
| BRL-010 | A canonical record's entity type must match one of the predefined entity types for its asset class — records with invalid column population patterns or contract formats are rejected on create/update | FR-NMD-001-015, FR-NMD-001-016 |
...
| EX-007 | Create or update attempted with contract format not matching the entity type's permitted formats | Reject; return validation error specifying permitted formats for the entity type; do not persist. Applies to Futures & Options entity types with a contract column (Future Contract, Option Reference Contract, Option Contract). FX records have no contract column — this exception does not apply to FX. |
| EX-008 | Create or update attempted with Product Type value not in the defined set for the asset class | Reject; return validation error listing permitted Product Type values; do not persist. Applies to Futures & Options only — FX has no Product Type enumeration defined in Phase 1. |
| EX-009 | Create or update of FX Currency Pair record attempted with Commodity value not in XXXYYY format (not exactly 6 uppercase characters) | Reject; return validation error specifying required XXXYYY format; do not persist |
...
|----------------|------------------|------------|
| FR-001 | FR-NMD-001-001, FR-NMD-001-002, FR-NMD-001-003, FR-NMD-001-004, FR-NMD-001-005, FR-NMD-001-006, FR-NMD-001-007, FR-NMD-001-008, FR-NMD-001-009, FR-NMD-001-010, FR-NMD-001-011, FR-NMD-001-012, FR-NMD-001-013, FR-NMD-001-014, FR-NMD-001-015, FR-NMD-001-016 | PSD, DC, UT |
...
| AC-011 | All 6 Futures & Options entity types (Exchange, Commodity, Product, Future Contract, Option Reference Contract, Option Contract) are representable in the canonical table with correct column population |
| AC-012 | Contract format validated per entity type — invalid formats rejected on create/update (EX-007); Option Contract format follows YYYYMM_CP_STRIKE pattern where CP is C or P and STRIKE is the strike price |
| AC-013 | Product Type values outside the defined set (PHY, FUT, CMB, OOP, OOF, OOC) are rejected for Futures & Options records (EX-008) |
| AC-014 | FX Currency Pair records have Exchange fixed as 'FX', Commodity in XXXYYY format, and columns 3-5 null; invalid currency pair formats rejected on create/update (EX-009) |
...
| OI-004 | FX Product Type enumeration not defined for Phase 1 — to be added in a future revision when FX product types are confirmed |
...
| v0.2 | 2026-03-01 | Leo | Added FA-03 Asset Class Canonical Definitions with FR-NMD-001-015 — predefines 6 Futures & Options entity types (Exchange, Commodity, Product, Future Contract, Option Reference Contract, Option Contract) with column population rules, contract format validation (YYYYMM | YYYYMMDD | YYYYMMWN), and Product Type enumeration (PHY, FUT, CMB, OOP, OOF, OOC). Added BRL-010, EX-007, EX-008, AC-011–013. Added entity_type field to Canonical Record. Updated scope, definitions, and traceability. |
| v0.3 | 2026-03-01 | Leo | Corrected Option Contract format in FR-NMD-001-015 from C/P_NN.NN notation to CP_STRIKE (where CP = C or P, STRIKE = strike price). Added FR-NMD-001-016 — FX asset class entity type: FX Currency Pair with Exchange fixed as 'FX' and Commodity in XXXYYY format, columns 3-5 null, no Product Types defined in Phase 1. Added EX-009 for FX currency pair format validation. Updated AC-012 with corrected Option Contract format; added AC-014 for FX Currency Pair. Updated BRL-010 to reference FR-NMD-001-016. Clarified EX-007 and EX-008 scope (Futures & Options only). Added OI-004 for deferred FX Product Types. |
